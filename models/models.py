...
class Encoder(nn.Module):
    def __init__(self, in_channels=3, depth=3):
        ...
        self.interpolate = Interpolation(
            5, 12, in_channels * (4**depth), act=relu)
...
class ResNet(nn.Module):
    def __init__(self, hidden_channels):
        ...
        self.res_block1 = ResidualBlockWithChannelAttention(
            hidden_channels, hidden_channels)
        self.res_block2 = ResidualBlockWithChannelAttention(
            hidden_channels, hidden_channels)
        self.res_block3 = ResidualBlockWithChannelAttention(
            hidden_channels, hidden_channels)
        self.res_block4 = ResidualBlockWithChannelAttention(
            hidden_channels, hidden_channels)
        self.res_block5 = ResidualBlockWithChannelAttention(
            hidden_channels, hidden_channels)
        self.conv_final = nn.Conv2d(
            hidden_channels, 3, kernel_size=1, stride=1, padding=0)
