...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Video Stabilization using CAIN')
    parser.add_argument('--in_path', type=str, help='Input video file path')
    parser.add_argument('--out_path', type=str,
                        help='Output stabilized video file path')
...
def stabilize(in_path: str, out_path: str):
    ...
    if ext not in ['.mp4', '.avi']:
        print(
            f"The input file '{in_path}' is not a supported video file (only .mp4 and .avi are supported).")
        exit()
    ...
    for iter in range(ITER):
        ...
        for frame_idx in range(2, frame_count - 2):
            ...
            ft_minus = torch.from_numpy(
                interpolated[frame_idx - 1, ...]).permute(2, 0, 1).unsqueeze(0).to(device)
            ft = torch.from_numpy(frames[frame_idx]).permute(
                2, 0, 1).unsqueeze(0).to(device)
            ft_plus = torch.from_numpy(
                interpolated[frame_idx + 1, ...]).permute(2, 0, 1).unsqueeze(0).to(device)
            with torch.no_grad():
                fout, features = cain(ft_minus, ft_plus)
                # refinement step
                if iter == 2:
                    ft_2 = torch.from_numpy(
                        frames[frame_idx - 2, ...]).permute(2, 0, 1).unsqueeze(0).to(device)
                    ft_1 = torch.from_numpy(
                        frames[frame_idx - 1, ...]).permute(2, 0, 1).unsqueeze(0).to(device)
                    ftplus1 = torch.from_numpy(
                        frames[frame_idx + 1, ...]).permute(2, 0, 1).unsqueeze(0).to(device)
                    ftplus2 = torch.from_numpy(
                        frames[frame_idx + 2, ...]).permute(2, 0, 1).unsqueeze(0).to(device)
                    resnet_input = torch.cat(
                        [ft_2, ft_1, fout, ftplus1, ftplus2], dim=1)
                    fout = resnet(resnet_input)
            temp[frame_idx, ...] = fout.cpu().squeeze(
    ...
    stable_frames = np.clip(
        (255 * (interpolated + 1) / 2), 0, 255).astype(np.uint8)
